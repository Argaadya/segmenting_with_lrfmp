...
-   `Periodicity`: The median number of days of customer's inter-visit times. This will change the perspective that instead of looking the regularity of customer visit time, we will look at the average inter-visit time between customer visits.
...
The x-axis shows the first dimension (PC1) from the PCA while the y-axis shows the second dimension (PC2) from the PCA. PC1 give us 41.8% of information while PC2 give us 26.8% of information, therefore the plot that we see contains around 69% of information from the data that we have while the remaining 31% of information is not presented. That's why some clusters are seen overlapped to each other perhaps because no additional information is presented. The red label and text indicated the direction of each LRFMP variables, with data that is on the direction of the arrow indicate higher value of the variable and vice versa. For example, the frequency arrow is directed to the upper left of the plot, and cluster 3 is also located on that direction. We know from the previous table that cluster 3 has highest `Frequency`. Meanwhile, cluster 4 which are located on the opposite of the frequency arrow has the least `Frequency`.

From the plot above we can see that the four segments of customer are almost perfectly separated with some segments still overlapping. Cluster 2 is the least recent (has high `Recency`) and also the least loyal (has low `Length`). Cluster 3 has highest `Frequency` and lowest `Recency` while cluster cluster 4 has lowest `Frequency`.

## Clustering LFRMP and Product Preferences
